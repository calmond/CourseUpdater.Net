...
internal static class CourseUpdater
{
    ...
    public static async Task<int> RunAsync(string[] args)
    {
        try
        {
            ...
            {
                return Fail(
                    "Usage: dotnet run --project tools/CourseUpdater -- [lesson-number|--list]");
            }
            ...
            if (!await RemoteExistsAsync(manifest.Remote))
            {
                return Fail(
                    ...
                    "Add the instructor repository as the configured remote before running the updater.");
            }
            ...
            if (cherryPick.ExitCode != 0)
            {
                ...
                return Fail(
                    ...
                    "Your repository has been returned to its previous state.\n" +
            ...
            var nextSteps = manifest.NextSteps ?? new[] { "git push" };
            if (nextSteps.Count > 0)
            {
                Console.WriteLine("Next steps:");
                foreach (var command in nextSteps)
                {
    ...
    private static async Task<string> GetRepositoryRootAsync()
    {
        ...
        if (result.ExitCode != 0)
        {
            throw new InvalidOperationException(
                "Run the course updater from inside the project Git repository.");
        }
    ...
    private static async Task<CourseManifest> LoadManifestAsync()
    {
        if (!File.Exists(ManifestPath))
        {
            throw new FileNotFoundException(
                $"Course update manifest not found: {ManifestPath}");
        }

        await using var stream = File.OpenRead(ManifestPath);

        var manifest = await JsonSerializer.DeserializeAsync<CourseManifest>(
            stream,
    ...
    private static LessonUpdate? SelectLesson(
        ...
        string[] args)
    {
        ...
        if (!int.TryParse(args[0], out var lessonNumber))
        {
            throw new ArgumentException(
                "Lesson must be specified as a number, or use --list.");
        }

        var lesson = manifest.Lessons.SingleOrDefault(
            candidate => candidate.Number == lessonNumber);

        if (lesson is null)
        {
            throw new ArgumentException(
                $"Lesson {lessonNumber} is not defined in {ManifestPath}.");
        }
    ...
    private static bool PrerequisitesInstalled(
        CourseManifest manifest,
    ...
    private static void PrintLessonStatus(CourseManifest manifest)
    {
        ...
        foreach (var lesson in manifest.Lessons.OrderBy(
                     lesson => lesson.Number))
        {
            var status = File.Exists(lesson.Marker)
                ? "Installed"
                : "Not installed";

            Console.WriteLine(
                $"{lesson.Number,2}. {lesson.Name,-36} {status}");
        }
    }
        ...
        params string[] arguments)
    {
        ...
        {
            throw new InvalidOperationException(
                "Git could not be started. Verify that Git is installed and available on PATH.",
                ex);
        }
